-- Revert modal_akhir to standard asset closing formula:
-- modal_akhir = final_physical_balance + bank_finals + ppob_finals
-- Setoran kasir (deposit_amount) is tracked separately and FS = Setoran * 15%.
-- Idempotent: safe to re-run.

-- 1) Recreate close_shift_atomic without deposit in modal_akhir
...
BEGIN
  ...
  -- Gross closing capital = cash in drawer + bank + PPOB
...
-- 2) Recreate owner_adjust_balance to not include deposit_amount in modal_akhir
...
-- 4) Revert historical closed shifts modal_akhir to standard asset closing
DO $$
BEGIN
  PERFORM set_config('brilink.audit_override', 'on', true);
