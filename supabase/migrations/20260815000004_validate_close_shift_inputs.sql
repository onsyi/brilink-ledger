-- Validate close-shift inputs server-side.
--
-- README §3.5 requires Deposit_Amount to be validated against Saldo Fisik Akhir,
-- but close_shift_atomic accepted any deposit: a cashier could record handing
-- over Rp 10.000.000 out of a Rp 500.000 drawer. The capital model counts the
-- drawer *before* the money is handed to the owner (labaFee uses
-- final_physical_balance without subtracting the deposit), so the invariant is
-- deposit <= final cash.
--
-- The negative-amount guards mirror the CHECK constraints, but raise a readable
-- Indonesian message instead of a raw constraint violation.
...
BEGIN
  ...
  IF COALESCE(_topup, 0) < 0 THEN
    RAISE EXCEPTION 'Permintaan top-up tidak boleh negatif';
  END IF;
  ...
  END IF;
  IF COALESCE(_deposit, 0) > COALESCE(_final_cash, 0) THEN
    RAISE EXCEPTION 'Setoran (%) melebihi saldo fisik akhir (%)',
      COALESCE(_deposit, 0), COALESCE(_final_cash, 0);
  END IF;
