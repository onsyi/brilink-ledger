...
CREATE UNIQUE INDEX transactions_idempotency ON public.transactions (shift_id, client_ref) WHERE client_ref IS NOT NULL;
...
CREATE TABLE public.ppob_balances (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  shift_id UUID NOT NULL REFERENCES public.shifts(id) ON DELETE CASCADE,
...
CREATE INDEX receivables_shift_idx ON public.receivables (shift_id);
